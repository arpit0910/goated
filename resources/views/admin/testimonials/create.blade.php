    ...
    <div class="card" style="max-width: 800px; padding: 3rem;">
        ...
        <form action="{{ route('admin.testimonials.store') }}" method="POST">
            ...
            <div style="margin-bottom: 2rem;">
                ...
                <input type="text" name="student_name" value="{{ old('student_name') }}" required style="width: 100%; background: #111; border: 1px solid var(--border); padding: 1.2rem; color: #fff;">
            </div>
            ...
            <div style="margin-bottom: 2rem;">
                ...
                <textarea name="content" rows="6" required style="width: 100%; background: #111; border: 1px solid var(--border); padding: 1.2rem; color: #fff;">{{ old('content') }}</textarea>
            </div>

            <div style="margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; gap: 1rem; cursor: pointer;">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', true) ? 'checked' : '' }} style="width: 20px; height: 20px; accent-color: var(--fg);">
                    <span style="font-family: 'Syncopate', sans-serif; font-size: 0.6rem; color: var(--fg); letter-spacing: 1px;">PUBLISH ON SITE</span>
                </label>
            </div>

            <button type="submit" class="btn btn-dark" style="padding: 1rem 3rem;">SAVE TESTIMONIAL</button>
